...
module BancardPixServiceClient
  class PaymentHooksApi
    ...
    # @return [ShowPaymentHook]
    def create_payment_hook(opts = {})
      data, _status_code, _headers = create_payment_hook_with_http_info(opts)
      data
    end
    ...
    # @option opts [PaymentHookCreateRequest] :payment_hook_create_request 
    # @return [Array<(ShowPaymentHook, Integer, Hash)>] ShowPaymentHook data, response status code and response headers
    def create_payment_hook_with_http_info(opts = {})
      if @api_client.config.debugging
      ...
      # http body (model)
      post_body = opts[:debug_body] || @api_client.object_to_http_body(opts[:'payment_hook_create_request'])
      ...
      data, status_code, headers = @api_client.call_api(:POST, local_var_path, new_options)
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: PaymentHooksApi#create_payment_hook\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
      return data, status_code, headers
    end
  end
